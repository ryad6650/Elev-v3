...
- [x] Édition quantité entrée nutrition
- [x] Favoris aliments persistés en DB
- [x] Pré-remplissage poids dernière séance (poidsRef)
- [x] Détection PR automatique + animation
...
### Édition quantité + Favoris + PoidsRef + PR (fait le 2026-04-03)

**Feature 1 — Édition quantité entrée nutrition :**
Fichiers créés :

- `src/components/nutrition/EditEntryModal.tsx` — modal édition quantité (FoodDetailSheet en mode edit)

Fichiers modifiés :

- `src/store/nutritionStore.ts` — ajout `updateEntry(id, quantiteG)` optimiste + rollback
- `src/components/nutrition/FoodDetailSheet.tsx` — props `initialQuantity`, `confirmLabel`, `isFavorite`, `onToggleFavorite`
- `src/components/nutrition/NutritionPageClient.tsx` — FoodViewSheet remplacé par EditEntryModal (tap sur aliment = éditer quantité)

**Feature 2 — Favoris aliments persistés en DB :**
Fichiers créés :

- `supabase/migrations/016_user_aliment_favorites.sql` — table + RLS + index

Fichiers modifiés :

- `src/components/nutrition/FoodDetailSheet.tsx` — bouton favori connecté à `onToggleFavorite` (état réel DB)
- `src/components/nutrition/AddFoodModal.tsx` — passe `isFavorite` + `onToggleFavorite` à FoodDetailSheet

**Feature 3 — Pré-remplissage poids dernière séance :**
Fichiers modifiés :

- `src/app/actions/routines.ts` — export `getExerciseLastRefs` (server action publique wrappant getLastSessionRefs)
- `src/store/workoutStore.ts` — `addExercise` accepte `poidsRef`/`repsRef`, passés à `buildSets`
- `src/components/workout/ExerciseSearch.tsx` — fetch refs en parallèle des rests avant d'ajouter un exercice en séance libre
- `src/components/workout/ActiveWorkout.tsx` — import getExerciseLastRefs pour remplacement exercice

**Feature 4 — Détection PR automatique + animation :**
Fichiers modifiés :

- `src/components/workout/ActiveWorkout.tsx` — auto-dismiss PR banner 4s, vibration haptic, animation slide-in, design amélioré (emoji 🏆, gradient)
- `src/app/globals.css` — ajout `@keyframes prSlideIn`

Décisions techniques :

- Édition quantité = optimistic update + rollback si erreur Supabase
- Favoris table = UNIQUE(user_id, aliment_id) + RLS complet
- PoidsRef fetchée en parallèle avec restDuration via Promise.all (pas de latence ajoutée)
- PR détecté en temps réel (poids > poidsRef) grâce au poidsRef maintenant disponible en séance libre
- navigator.vibrate pour feedback haptic sur PR (Android + PWA)
