...
function matchFiltre(p: Programme, filtre: string): boolean {
  ...
  if (filtre === 'PPL') return txt.includes('ppl') || (txt.includes('push') && txt.includes('pull'));
  if (filtre === 'Full Body') return txt.includes('full');
  if (filtre === 'Upper / Lower') return txt.includes('upper') || txt.includes('lower');
...
export default function ProgrammesPageClient({ data }: Props) {
  ...
  const filtres = FILTRES.filter((f) => f === 'Tous' || data.programmes.some((p) => matchFiltre(p, f)));
      ...
      {data.programmes.length === 0 && (
        <div className="text-center py-16">
          ...
          <h2
            ...
            style={{ fontFamily: 'var(--font-dm-serif)', fontStyle: 'italic', color: 'var(--text-secondary)' }}
          >
            Aucun programme
          </h2>
