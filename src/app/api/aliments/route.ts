...
async function searchOFF(q: string, timeoutMs = 1500): Promise<NutritionAliment[]> {
  try {
    const params = new URLSearchParams({
      action: "process", search_terms: q, json: "1", page_size: "8",
      sort_by: "unique_scans_n", fields: "code,product_name,product_name_fr,brands,nutriments",
    });
...
async function searchByBarcode(
  ...
  const { data } = await supabase.from("aliments").select(SELECT_COLS)
    .eq("code_barres", barcode).limit(1);
  if (data?.length) return [{ ...data[0], source: "local" as const }];
  try {
    const res = await fetch(
      `https://world.openfoodfacts.org/api/v0/product/${barcode}.json`,
      { headers: { "User-Agent": "Elev-v3/1.0" }, signal: AbortSignal.timeout(5000) }
    );
