...
export async function fetchProgrammesData(
  ...
  const [profileRes, programmesRes, routinesRes] = await Promise.all([
    ...
    supabase
      .from("programmes")
      .select(
        "id, nom, description, difficulte, duree_semaines, jours, programme_routines(jour, routines(id, nom, routine_exercises(id)))",
      )
      .eq("user_id", user.id)
  ...
  if (profile?.programme_actif_id) {
    const found =
      programmes.find((p) => p.id === profile.programme_actif_id) ?? null;
    if (found) {
      let progres: Programme["progres"] | undefined;
      if (profile.programme_actif_debut) {
        const diffDays = Math.floor(
          (Date.now() - new Date(profile.programme_actif_debut).getTime()) /
            86400000,
        );
        progres = {
          semaine_actuelle: Math.min(
            Math.floor(diffDays / 7) + 1,
            found.duree_semaines,
          ),
  ...
  return {
    programmes,
    programmeActif,
    routinesDisponibles: (routinesRes.data ?? []) as {
      id: string;
      nom: string;
    }[],
  };
